<template>
    <article class="page-merge">
        ...
        <div class="page-merge__middle">
            ...
            <div class="mt-2">
                <button class="a-button a-button--primary" @click="$router.push({path:'/list'})">返回</button>
                <h1>订单信息</h1>
                <h3>{{ $store.state.mergeData.sku }}</h3>
                <h3>{{ $store.state.mergeData.variant_title }}</h3>
                <h2 class="text-danger">数量: {{ $store.state.mergeData.quantity }}</h2>
            </div>
